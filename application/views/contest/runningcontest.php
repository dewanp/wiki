	...
	<div class="feeds-main-block">
        ...
        <span class="title">
						<?php 
                               ...
                               echo anchor(getContestUrl($post['contest_id'], $type = 1),$list_title);
                        ?>
      	</span>
...
<?php if(!empty($posts) && count($posts) >= 10){ ?>
     <div class="txtshow showmore" id="show-post-more"> 
         <a href="javascript:void(0);" class="txtgrey" onclick="loadMoreContest('runningcontest');"><span class="txtload">Show more</span> 
...
<?php if(empty($posts) && count($posts) == 0) { ?>
	<div style="padding: 15px;">
        <div id="zip-info-box" class="info-block test">	
                <span class="info-show">There are no more Running Contest available right now for you.</span>
        </div>
    </div>
<?php }?>