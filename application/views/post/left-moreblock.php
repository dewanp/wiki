<?php if(!empty($post_capsules)){?>
	...
	{
		var capsule_type_dis = capsule_type; 
		if($("#selected-elements .rows").length<6)
		{
		
			if(capsule_type=='opinion')
			{
				capsule_type_dis = "reviews";
			}
			
			var selectedHtml = '<div class="rows"><a href="javascript:void(0);" style="text-transform:capitalize;" class="'+capsule_type+'">'+capsule_type_dis+'</a><a href="javascript:void(0);" class="add" onclick="$(this).parent().remove();"></a><input type="hidden" name="post_new_capsules[]" value="'+capsule_data+'" /></div>';		
...
<div class="btnbox"> 
	<a onclick="saveDraftAddBlock('capsule-wrapper');openPopDiv('add-blocks');" class="btnorange" href="javascript:void(0);" hidefocus="true" style="outline: medium none;">Add more blocks</a> 
</div>
			...
			<div class="addmore-capsule-list">
				<?php foreach($capsules as $capsule){?>
					<?php if($capsule['name']!='comment'){?>
						<div class="rows">
							<a href="javascript:void(0);" class="<?php echo $capsule['name']?>"><?php  echo ucfirst($capsule['name']=='opinion'?'reviews':$capsule['name']);?></a>
