...
<div class="comment-form-wrapper comm-inner">
	<form name="capsuleForm<?php echo $capsule_id?>" id="capsuleForm<?php echo $capsule_id?>">
		...
		<div class="commfield">
			...
			<textarea cols="80" rows="3" name="comment_description" id="comment_description" class="inputmain" onfocus="$(this).removeClass('error-border');" maxlength="300"></textarea>
...
<div class="showcomment" style="width:695px;">	
	<?php foreach($capsule_content as $capsule_comment){?>
		...
		<a href="<?php echo site_url($capsule_comment['user_name']);?>" title="<?php if($capsule_comment['profile_name']) echo $capsule_comment['profile_name']; else echo $capsule_comment['user_name'];?>">
			<div class="scthumb" id="scthumb-comment-<?php echo $capsule_comment['comment_id']?>">
				<script> showImage('<?php echo $capsule_comment["picture"]?>','30','30','scthumb-comment-<?php echo $capsule_comment["comment_id"]?>');</script>
			</div>
		</a>
		<div class="sctxt">
			<h4>
			...
			if($capsule_comment["user_name"]){
				$commuser = anchor($capsule_comment["user_name"],$capsule_comment["user_name"]);
			}
			if($capsule_comment["profile_name"]){
				$commuser = anchor($capsule_comment["user_name"],$capsule_comment["profile_name"]);
			}
			echo $commuser?></h4>
