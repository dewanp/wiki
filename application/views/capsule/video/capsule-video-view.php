				...
				<div class="ad-thumbs">
					<ul class="ad-thumb-list">
						<?php foreach($capsule_content as $r =>$capsule_content_row){?>
							...
							<?php }else{?>
                            	<li><a id="video-thumb-<?php echo $r?>" href="javascript:void(0);" onclick="showvideo('cur-video-id-<?php echo $capsule_content_row['capsule_id']?>','<?php echo S3_URL.BUCKET_NAME.'/uploads/'.$capsule_content_row['file_path'];?>')">
                                <img src="<?php echo S3_URL.BUCKET_NAME.'/uploads/'.$capsule_content_row['file_path']?>.jpg" alt="" width="113" height="72" class="image<?php echo $r?>"/></a></li>
                            <?php }?>
						<?php }	?>
					</ul>
				</div>
