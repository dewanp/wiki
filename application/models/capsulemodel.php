...
class Capsulemodel extends CI_Model {
	...
	function capsuleDetail($post_id, $capsule_type='' , $view_type = ''){
		...
		if($view_type == 'view'){
			...
			if($capsule_type){
				$capsules =	   $this->db->select('capsule.*, capsule_type.name as capsule_name')
									...
									->where(array('post_id'=>$post_id, 'capsule_type.name'=>$capsule_type, 'capsule.capsule_type_id !='=>5))
									->order_by("weight", "asc")
									->get();
			}else{
				$capsules =	   $this->db->select('capsule.*, capsule_type.name as capsule_name')
									->from('capsule')
									->join('capsule_type', 'capsule.capsule_type_id = capsule_type.capsule_type_id', 'left')
									->where(array('post_id'=>$post_id,'capsule.capsule_type_id !='=>5))
									->order_by("weight", "asc")
									->get();
	...
	/* This function is used for the comment wrapper 16 january 2013. */
	function capsuleCommentDetail($post_id, $capsule_type='' , $view_type = ''){
