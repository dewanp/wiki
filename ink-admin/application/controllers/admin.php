...
class Home extends CI_Controller
{
	...
	function __construct()
	{
        ...
        $this->load->model(array('adminmodel','commonmodel'));
	...
	*/
	...
	public function index()
	{
		...
		$this->load->view('include/header-login', $data);
	...
	*/
	...
	{ 
		...
		else
		{
			...
			$res = $this->adminmodel->checkLogin($username, $password) ;
...
*/
	...
	*/
	...
	{		
		if($this->adminmodel->isLoggedIn()){
			$data['summary'] = $this->adminmodel->RegistrationSummary();
			$data["result"] = $this->adminmodel->HighestGrosser();
           	$data['title'] = "Vinfotech-wiki Admin Section";
			$data['active'] ="home";
			$this->load->view('include/header', $data);
	...
	*/
	...
	public function manageUsers()
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$config['base_url'] = site_url("home/manageusers/$search_user");
			...
			$data['manage_user'] = $this->adminmodel->ManageUser($limit ,$start,$search_user);
	...
	*/
	...
	public function suspendedUserList()
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$config['base_url'] = site_url("home/suspendeduserlist/$search");
			$config['total_rows'] =  $all ;
			$config['per_page'] = $limit;
			...
			$data['suspended_user'] = $this->adminmodel->suspendedUserList($limit,$start,$search);
			$data['title'] = "Vinfotech-wiki Admin Section";
			$data['active'] ="manage_user";
			$data['count_user'] = $all;
			$data['start'] = $start;
	...
	*/
	...
	{
		if($this->adminmodel->isLoggedIn()){
			$user_detail = $this->adminmodel->manageUserViewDetails($user_id);
			$data['title'] = "Vinfotech-wiki Admin Section";
			$data['active'] ="manage_user";
	...
	*/
	...
	public function manageUsersLoginHistory($user_id)
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$start = $this->uri->segment(4,$start);
			$config['base_url'] = site_url("home/manageusersloginhistory/".$user_id);
			$config['total_rows'] =  $all ;
			$config['per_page'] = $limit;
			$config['uri_segment'] = '4' ;
			$config['num_links'] = '2';
	...
	*/
	public function manageUsersContentHistory($user_id)
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$start = $this->uri->segment(4,$start);
			$all =$this->db->count_all_results();
			$config['base_url'] = site_url("home/manageuserscontenthistory/".$user_id);
			$config['total_rows'] =  $all ;
			$config['per_page'] = $limit;
			$config['uri_segment'] = '4' ;
			$config['num_links'] = '2';
	...
	*/
	 ...
	 */
	 ...
	 function resumeSuspendedUsers($user_id="",$RedirectPage='0',$activateManually='0')
	 {
		 ...
		 if($RedirectPage != 1)
		 {
			 redirect('home/suspendeduserlist');
		 }
		 else
		 {
			//load user status section using ajax from user detail page in admin ($RedirectPage wou1d be 1 for this case)
	...
	*/
	...
	{	
		if($this->adminmodel->isLoggedIn()){
			...
			$this->load->view('include/header', $data);
			$this->load->view('manage-users-view-details');
			$this->load->view('include/footer');
			}
		else
			{
	...
	function updateUserData()
	{
		if($this->input->post('save')!= null){			 			 
				...
				$this->form_validation->set_rules('user_name', 'User name', 'trim|required|xss_clean');
				 ...
				 if( $this->form_validation->run() == FALSE )
				{
					$user_detail = $this->adminmodel->manageUserViewDetails($this->session->userdata('user_id'));
					$data['title'] = "Vinfotech-wiki Admin Section";
					$data['active'] ="manage_user";
					$data['user_detail'] = $user_detail;
				...
				else
				{
					...
					redirect('home/manageuserseditdetails/'.$this->session->userdata('user_id'));
				}
	...
	public function  viewPostMultimedia($post_id)
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$data =array();
			$data['all_result'] = $this->adminmodel->viewPostMultimedia($post_id);
	...
	public function editPost($post_id)
	{
		...
		// post capsules		
		$data['post_capsules'] =  $this->adminmodel->capsuleDetail($post_id);
 ...
 */
	public function updateCapsuleWrapper(){
		...
		$data['post_id'] = $post_id;
		$data['post_capsules'] =  $this->adminmodel->capsuleDetail($post_id, $capsule_type);
...
*/
	...
	public function manageContent()
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$config['base_url'] = site_url("home/managecontent");
			$config['total_rows'] =  $all ;
			$config['per_page'] = $limit;
		    ...
		    $data['all_posts'] = $this->adminmodel->manageContent($start,$limit,0);
	...
	public function allBlockedPosts()
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$start = $this->uri->segment(3,$start);
			$config['base_url'] = site_url("home/allblockedposts");
			$config['total_rows'] =  $all ;
			$config['per_page'] = $limit;
			$config['uri_segment'] = '3' ;
			$config['num_links'] = '2';
		    ...
		    $data['all_blocked_posts'] = $this->adminmodel->manageContent($start,$limit,1);
			...
			$this->load->view('include/header', $data);
			$this->load->view('manage-content-blocked-posts');
			$this->load->view('include/footer');
		}
		else
		{
	...
	/* This function is used for load view of categor list.	*/
	 function displayCategoryList()
	 {
		 if($this->adminmodel->isLoggedIn()){
			...
			$config['base_url'] = site_url("home/displaycategorylist");
			$config['total_rows'] =  $all ;
			$config['per_page'] = $limit;
			$config['uri_segment'] = '3' ;
			$config['num_links'] = '2';
			...
			$data['category_list'] = $this->adminmodel->displayCategoryChildList($limit,$start);
	 ...
	 function Category_status_change($category_id="" , $status="")
	 {
		  $is_active_array = $this->input->post('is_active');
			if($this->input->post('active')== "Activate Selected")
			 ...
			 redirect('home/displaycategorylist');
 	 }
...
*/
	...
	function displayAddCategory()
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$data['category_result'] = $this->adminmodel->displayCategoryDropdown();
		...
		if($this->input->post('add_category') == 'Add New Category')
		{
			...
			else
			{
				...
				if( !empty($admins) && !empty($parent) )
				{
					...
					$parent_admin = $this->adminmodel->getAdminCategories($parent);
	...
	*/
	/*public function displayEditCategory($category_id)
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$category_detail = $this->adminmodel->displayEditCategory($category_id);
			$data['title'] = "Vinfotech-wiki Admin Section";
			$data['active'] ="category";
			...
			$inherited_user = $this->db->select('user_id')->from('user_category_relation')->where('category_id',$category_id)->where('permission_type',1)->where('is_inherited',1)->get();
			
			$inherited_user_ids = array();
			foreach($inherited_user->result_array() as $inhertedval)
			{
				$inherited_user_ids[] = $inhertedval['user_id'];
			}
			$data['inherited_userr'] = $inherited_user_ids;
	...
	/*public function editCategory()
	{
		if($this->input->post('save') =='Save')
		{
			...
			if( $this->form_validation->run() == FALSE )
			{
				...
				$admin_user = $this->db->select('user_id')->from('user_category_relation')->where('category_id',$category_id)->where('permission_type',1)->get();
				
				$admin_ids = '';
				foreach($admin_user->result_array() as $adminval)
				{
					$admin_ids .= $adminval['user_id'].',';
				}
				$data['admin_userr'] = $admin_ids;
				
				
				$rw_user = $this->db->select('user_id')->from('user_category_relation')->where('category_id',$category_id)->where('permission_type',2)->get();	
				$rw_ids = '';
				foreach($rw_user->result_array() as $rwval)
				{
					$rw_ids .= $rwval['user_id'].',';
				}
				$data['rw_userr'] = $rw_ids;
				
				
				$r_user = $this->db->select('user_id')->from('user_category_relation')->where('category_id',$category_id)->where('permission_type',3)->get();		
				$r_ids = '';
				foreach($r_user->result_array() as $rval)
				{
					$r_ids .= $rval['user_id'].',';
				}
				$data['r_userr'] = $r_ids;
			...
			else
			{
				...
				redirect(base_url().'home/displayEditCategory/'.$category_id);
			}
		}
		else
		{
   ...
   function displayPostTypeList()
	{
	    if($this->adminmodel->isLoggedIn()){
			...
			$data['post_type_list'] = $this->adminmodel->displayPostTypeList($limit,$start);
			$data['title'] = "Vinfotech-wiki Admin Section";
			$data['active'] ="post_type";
			$data['count_user'] = $all;
	
			$this->load->view('include/header', $data);
			$this->load->view('post-type-list');
			$this->load->view('include/footer');
		}
		else
		{
   ...
   function  displayAddSubCategory()
	{
	   if($this->adminmodel->isLoggedIn()){
			$data['title'] = "Vinfotech-wiki Admin Section";
			$data['active'] ="post_type";
			$data['capsule_list'] = $this->adminmodel->capsuleTypeList();
	  ...
	  if($this->input->post('add_user') == 'Add New Sub Category')
		{
			...
			{
				$this->adminmodel->addNewPostTypeList();
				redirect('home/displayposttypelist');
			}
		}
		else
		{
...
*/
	...
	{
		if($this->adminmodel->isLoggedIn()){
			$sub_category_detail = $this->adminmodel->displayEditSubCategory($sub_category_id);
			$data['title'] = "Vinfotech-wiki Admin Section";
			$data['active'] ="post_type";
			...
			$this->load->view('include/header', $data);
			$this->load->view('edit-sub-category');
			$this->load->view('include/footer');
		}
		else
		{
	...
	public function editSubCategory()
	{
		if($this->input->post('save') == 'Save')
		{
			...
			{								
				...
				$this->adminmodel->editSubCategory($sub_category_id);
				redirect('home/displayposttypelist');
			}
		}
		else
...
/* Google Ad Sense  */
		...
		if($this->adminmodel->isLoggedIn())
			{
				...
				$this->load->view('include/header', $data);
				$this->load->view('update-google-ads');
				$this->load->view('include/footer');
	...
	*/
	...
	public function reportAbuseList()
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$config['base_url'] = site_url("home/reportabuselist");
			$config['total_rows'] =  $all ;
			$config['per_page'] = $limit;
			$config['uri_segment'] = '3' ;
			$config['num_links'] = '2';
			...
			$data['abuse_list'] = $this->adminmodel->reportAbuseList($limit,$start );
			$data['title'] = "Vinfotech-wiki Admin Section";
			$data['active'] ="report_abuse";

			$this->load->view('include/header', $data);
			$this->load->view('report-abuse');
			$this->load->view('include/footer');
	...
	*/
	...
	public function reportAbusedPost($post_id)
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$data['redirect_url'] =  $this->session->userdata('redirect_url') ? $this->session->userdata('redirect_url') : site_url('home/reportabuselist');

			$this->load->view('include/header', $data);
			$this->load->view('report-abuse-detail');
			$this->load->view('include/footer');
	...
	public function viewComments()
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$all =  count($this->adminmodel->viewComments($start,$limit,$post_id,true));
				
			$config['base_url'] = site_url("home/viewComments");
			$config['total_rows'] =  $all ;
			$config['per_page'] = $limit;
			$config['uri_segment'] = '4' ;
			$config['num_links'] = '2';
		    ...
		    $data['all_posts'] = $this->adminmodel->viewComments($start,$limit,$post_id);
			...
			$this->load->view('include/header', $data);
			$this->load->view('view-comments',$data);
			$this->load->view('include/footer');
		}
		else
		{
	...
	public function deleteComments($comment_id='',$post_id='')
	{
		if($this->adminmodel->isLoggedIn()){
			if(is_numeric($comment_id)){
				$this->adminmodel->deleteComments($comment_id);
				  //$all_posts = count($this->adminmodel->viewComments('','',$post_id,true));line to delete
				redirect("/home/viewComments/$post_id");	
			}
		}
		else
		{
	...
	*/
	...
	public function contactdetail()
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$start = $this->uri->segment(3,$start);			
			$config['base_url'] = site_url("home/contactdetail");
			$config['total_rows'] =  $all ;
			$config['per_page'] = $limit;
			$config['uri_segment'] = '3' ;
			$config['num_links'] = '2';
			...
			$data['contact_list'] = $this->adminmodel->contactDetailList($limit,$start);
	...
	*/
	...
	public function subscribeduser()
	{
		if($this->adminmodel->isLoggedIn()){
			...
			$start = $this->uri->segment(3,$start);
			$config['base_url'] = site_url("home/subscribeduser");
			$config['total_rows'] =  $all ;
			$config['per_page'] = $limit;
			$config['uri_segment'] = '3' ;
			$config['num_links'] = '2';
			...
			$data['subscribed_user'] = $this->adminmodel->subscribedUserList($limit,$start);
			...
			$this->load->view('include/header', $data);
			$this->load->view('subscribed-user');
			$this->load->view('include/footer');
	...
	*/
	public function showcontestlisting()
	{
		if($this->adminmodel->isLoggedIn())
		{
			...
			$start = $this->uri->segment(3,$start);
			$config['base_url'] = site_url("home/showcontestlisting");
			$config['total_rows'] =  $all ;
			$config['per_page'] = $limit;
			$config['uri_segment'] = '3' ;
			$config['num_links'] = '2';
			...
			$data['contest_list'] = $this->adminmodel->contestList($limit,$start);
			...
			$this->load->view('include/header', $data);
			$this->load->view('contest/list-contest');
			$this->load->view('include/footer');
	...
	*/
	public function displayaddcontest()
	{
		...
		{			
			...
			$this->load->view('include/header', $data);
			$this->load->view('contest/add-contest');
			$this->load->view('include/footer');
		}
		else
		{
	...
	public function addcontest()
	{
		...
		if($this->input->post('add_contest')== "Add New Contest")
		{
			...
			{ 
					...
					redirect('home/showcontestlisting');
				}
					$data['title'] = "Vinfotech-wiki Admin Section";
					$data['active'] ="contests";
	...
	*/
	...
	{
		if($this->adminmodel->isLoggedIn()){
			$contest_detail = $this->adminmodel->manageContestEditDetails($contest_id);
			...
			$this->load->view('include/header', $data);
			$this->load->view('contest/edit-contest');
			$this->load->view('include/footer');
		}
		else
		{
	...
	*/
	public function contestview($contest_id)
	{
		if($this->adminmodel->isLoggedIn())
		{
			...
			$this->load->view('include/header', $data);
			$this->load->view('contest/view-contest');
			$this->load->view('include/footer');
		}
		else
		{
	...
	*/
	public function contestviewdetail($contest_id)
	{
		if($this->adminmodel->isLoggedIn())
		{
			...
			$data['manage_user'] = $this->adminmodel->ManageUserContest();
			...
			$this->load->view('include/header', $data);
			$this->load->view('contest/view-contest-detail');
			$this->load->view('include/footer');
		}
		else
		{
	...
	{		
	  ...
	  foreach($sub_arr as $key => $subArray){
		...
		foreach($newArray as $key => $val)
		{
			...
			/*if($key != 0 &&  $val != 0)
			{
				$data_array = array('user_id' => $key, 'post_id' => $val, 'contest_id' => $contest_id);
				$save_winner = $this->commonmodel->commonAddEdit('contest_winners', $data_array);
			}*/
		}
		redirect('home/showcontestlisting');
	}
	...
	public function getAdminInfo()
	{
		...
		$rec_adm = $this->adminmodel->getAdminInfo($category_id,1);
		$rec_rw = $this->adminmodel->getAdminInfo($category_id,2);
		$rec_r = $this->adminmodel->getAdminInfo($category_id,3);
...
// End of  home.php  controller file 
